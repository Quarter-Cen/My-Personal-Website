...
const Navbar: React.FC<NavbarProps> = ({ currentIndex }) => {
    const [Color, setColor] = useState("");
    const [HamColor, setHamColor] = useState("");
    const [isSmScreen, setIsSmScreen] = useState(false);

    const [FC, setFC] = useState(false);
    const [IGC, setIGC] = useState(false);
    const [GHC, setGHC] = useState(false);
    ...
    const getLinkClass = (path: string) => {
        return pathname === path ? 'text-[#72383D]' : 'text-zinc-900';
    };
    ...
    useEffect(() => {
        const handleResize = () => {
            if (window.innerWidth >= 640) { // ตรวจสอบว่าขนาดหน้าจอเป็น sm หรือใหญ่กว่า
                setIsSmScreen(true);
            } else {
                setIsSmScreen(false);
            }
    ...
    useEffect(() => {
        ...
        const isAboutSection = currentIndex === 1 || currentIndex === 2 || currentIndex === 3;
        ...
        setHamColor(svgHamColor)
        setColor(svgFillColor)
        }
    },[currentIndex, isSmScreen])
            ...
            <div className="navbar text-white">
                <div className="navbar-start h-[3.3rem] border-zinc-900 w-5/6">
                    <div className="border-l-[1px] h-[3.1rem] w-[5rem] border-b-[1px] border-zinc-800 grid place-items-center">
                        <img src="Logo2.png" className='h-[2.5rem]' alt="Logo" />
                    </div>
                    <div className="hidden sm:flex justify-center space-x-4 border-zinc-800 border-l-[1px] border-b-[1px]">
                ...
                <div className="mx-1 hidden md:flex">
                    <a href="https://www.facebook.com/kivye.j" aria-label="Visit my Facebook" target="_blank" rel="noopener noreferrer" className="hover:scale-110 transition-transform duration-300 ease-in-out " onMouseEnter={() => setFC(true)} onMouseLeave={() => setFC(false)}>
                        <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="36" height="36" viewBox="0 0 72 72" className={`transition-colors ${(FC) ? 'fill-[#72383D]' : Color}`}>
                    ...
                    <div className="mx-1 hidden md:flex">
                        <a href="https://www.instagram.com/jsnk_p/" aria-label="Visit my Instragram" target="_blank" rel="noopener noreferrer" className="hover:scale-110 transition-transform duration-300 ease-in-out hover-fill-important" onMouseEnter={() => setIGC(true)} onMouseLeave={() => setIGC(false)}>
                            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="36" height="36" viewBox="0 0 64 64" className={`transition-colors ${(IGC) ? 'fill-[#72383D]' : Color}`}>
                    ...
                    <div className="mx-1 hidden md:flex">
                        <a href="https://github.com/Quarter-Cen" aria-label="Visit my GitHub" target="_blank" rel="noopener noreferrer" className="hover:scale-110 transition-transform duration-300 ease-in-out " onMouseEnter={() => setGHC(true)} onMouseLeave={() => setGHC(false)}>
                            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="36" height="36" viewBox="0 0 32 32" className={`transition-colors ${(GHC) ? 'fill-[#72383D]' : Color}`}>
                    ...
                    <div className="mx-1 hidden md:flex">
                        <a href="https://www.youtube.com/@aunchan5668" aria-label="Visit my YouTube" target="_blank" rel="noopener noreferrer" className="hover:scale-110 transition-transform duration-300 ease-in-out " onMouseEnter={() => setYTC(true)} onMouseLeave={() => setYTC(false)}>
                            <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" width="36" height="36" viewBox="0 0 72 72" className={`transition-colors ${(YTC) ? 'fill-[#72383D]' : Color}`}>
